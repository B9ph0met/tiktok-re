 ...
 * X-Gnarly Generator
 * 
 * Based on the reverse-engineered implementation from justbeluga/tiktok-web-reverse-engineering.
...
function initPrngState() {
  ...
  return [
    ...
    aa[3], aa[70], aa[50], aa[32], aa[0] & nowMs,
    crypto.randomInt(aa[77]), crypto.randomInt(aa[77]), crypto.randomInt(aa[77]),
  ];
}
...
/* ── X-GNARLY ENCRYPT ──────────────────────────────────── */
function generateXGnarly(queryString, body = '', userAgent, envcode = 0, version = '5.1.1', timestampMs = Date.now()) {
  const obj = new Map();
  obj.set(1, 1);
  obj.set(2, envcode);
...
const signature = generateXGnarly(queryString, body, userAgent);
